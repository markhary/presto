...
```
sh modules/restaurant/server/scripts/database/sequelize-seed-database.sh
```
...
- **Menu** - Table holding relationship of items to each other.  An item where Modifies is NULL is a *top-level* item.  This table allows for a one-to-many relationship.  Care must be taken (and code introduced) to prevent [cyclic dependencies](#future-work).  Ultimately this relationship table defines the response to the API challenge.
...
---------|-------------|-------------------|----------|------------
(pk) int | (fk) ItemID | (fk) RestaurantID | itemID   | string 
